...
public final class Lexer
{
    ...
    public int getLineIndentation(int lineNum)
    {
        ...
        int i = lineStart;
        while (i != _charCodes.length && SourceCharacter.isWhiteSpace(_charCodes[i]))
        {
            ++i;
        }
        return i - lineStart;
    ...
    private void normalScan()
    {
        ...
        else if (SourceCharacter.isDecimalDigit(ch))
        {
        ...
        else
        {
            ...
            switch (ch)
            {
                case 0x2e:
                {
                    if (SourceCharacter.isDecimalDigit(lookahead(1)))
                    {
    ...
    private boolean scanComment()
    {
        ...
        if (ch == 0x2f)
        {
            ...
            while (hasRemaining() && !SourceCharacter.isLineTerminator(lookahead(0)))
            {
                shiftCharCode();
            }
    ...
    private void scanNumericLiteral(int ch, boolean onDot)
    {
        ...
        if (onDot)
        {
            ...
            while (SourceCharacter.isDecimalDigit(lookahead(0)));
        }
        else
        {
            if (ch == 0x30)
        ...
        if (ch == 0x65 || ch == 0x45)
        {
            ...
            if (!SourceCharacter.isDecimalDigit(lookahead(0)))
            {
                throw reportUnexpectedCharacter();
            }
            while (SourceCharacter.isDecimalDigit(lookahead(0)))
    ...
    private void scanHexLiteral()
    {
        ...
        int digit = -1;
        while ((digit = SourceCharacter.measureHexDigit(lookahead(0))) != -1)
        {
            value = (value << 4) | digit;
            shiftCharCode();
        }
    ...
    {
        var lead_line = lines.pop();
        int indent = 0;
        int i = 0;

        for (i = 0; i != lead_line.length(); ++i)
        {
            if (!SourceCharacter.isWhiteSpace(lead_line.codePointAt(i)))
            {
                break;
            }
        }
        ...
        for (int j = 0; j != lines.size(); ++j)
        {
            ...
            {
                if (!SourceCharacter.isWhiteSpace(line.codePointAt(i)) || i >= indent)
                {
                    break;
                }
            }
        ...
        switch (str.length())
        {
            ...
            case 5:
                ...
                return  str.equals("break") ? Token.BREAK :
                        ...
                        str.equals("yield") ? Token.YIELD :
                        str.equals("await") ? Token.AWAIT : null;
            case 6:
    ...
    {
        int value = SourceCharacter.measureHexDigit(lookahead(0));
        if (value == -1)
        {
    ...
    private void xmlTagScan()
    {
        ...
        else if (SourceCharacter.isXMLNameStart(ch))
        {
            do
            {
                shiftCharCode();
            }
            while (SourceCharacter.isXMLNamePart(lookahead(0)));
